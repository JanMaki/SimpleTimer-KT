...
class BCDiceManager {
    ...
    fun printInfo(channel: TextChannel){
        ...
        val gameSystemInfo = Json.decodeFromString(GameSystemInfo.serializer(), String(gameSystemInfoResponse.second.data))
        ...
        if (id != "DiceBot"){
            ...
            val defaultGameSystemInfo = Json.decodeFromString(GameSystemInfo.serializer(), String(defaultGameSystemInfoResponse.second.data))
    ...
    fun roll(channel: TextChannel,command: String, messageReply: MessageReply){
        ...
        if (commandPattern.containsMatchIn(runCommand)){
            ...
            //正常に処理されたかを確認
            val check = Json{ignoreUnknownKeys = true}.decodeFromString(OK.serializer(), String(rollPost.second.data))
            if (check.ok){
                //正常だった場合、結果を代入する
